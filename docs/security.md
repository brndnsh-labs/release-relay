...
Generated drafts carry explicit provenance: provider, model and configuration identity, and the source of the generated-at time. Revision lineage is carried separately: each human edit is stored as a distinct revision linked to its parent via `parentRevisionId`, never relabeled as provider output.
