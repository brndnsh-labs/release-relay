...
| M6 — Breakscope canary | Active | The v1 offline comparator is complete; v2 normalization and the v2 expectation-level comparator are complete (#52). The first operational scan is blocked on a reviewed source-free Breakscope export path, healthy production preflight, and a fresh approval immediately before any repository-selection change. |
