---
name: dep-update
description: Routine dependency hygiene for release-relay — survey what's outdated, plan the bump, update, run the gates, and land one dependency-metadata commit. Distinguishes a stale test expectation from a real regression. Never uses `audit fix --force`; never auto-pushes. Plan-first. Usage `/dep-update` (or `/dep-update <package>`).
---
<!-- cycle:rendered template=skills/dep-update.md.tmpl hash=ba4ca798e7c1 — managed by the-cycle; edit the template, not this file -->
...
1. **Survey.** `pnpm outdated` and `pnpm audit`. Group by risk: patch/minor on a leaf dependency is
...
3. **Branch** (§9).
4. **Update.** `pnpm update` for the routine set; name specific packages for anything riskier.
...
6. **Advisories.** `pnpm audit --fix` only if needed — **never a force
...
8. **Commit** (§8) — one commit, explicit paths (`package.json`, `pnpm-lock.yaml`, plus any test file a
...
- **Lockfile drift with nothing outdated:** `pnpm install` rewrote `pnpm-lock.yaml` without any
