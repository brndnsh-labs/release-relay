...
2. **Partition by status label** (§1): pickable · in flight (note, don't re-pick) ·
   ...
   promoted.
3. **Rank the pickable issues** by the §1 rule: **milestone first**, then **issue number**; only the earliest milestone with dependency-clear ready work is active.
